...
```{r, echo=false}
# df_melted_sorted <- melted %>% arrange(Var2, desc(melted$value_abs)) %>%  group_by(Var2) %>% slice(1:4)
# important_vars_name <- unique(df_melted_sorted$Var1)
```
...
df_final_aux <- cbind(df_final$Comp.2, df_final$Comp.1)
# clprofiles(clusters_final, df_final_aux)
```
