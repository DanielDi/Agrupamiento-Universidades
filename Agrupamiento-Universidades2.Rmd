...
plot(main_pca, main = "Varianza por cada componente",xlab="Componentes Principales", col="aquamarine",las = 2, cex.names = 0.7)
```
...
plot(df_final$Comp.1, df_final$Comp.2, col=df_final$clus, xlab="Componente Principal 1", ylab="Componente Principal 2", main = "Componente 1 vs Componente 2", pch = c(15, 16, 17, 18, 19)[as.numeric(df_final$clus)])
#legend("topleft", legend=c("1", "2","3","4"),title="Clusters", col=df_final$clus)
legend("topleft", legend = paste("Cluster", 1:4), pch=c(15, 16, 17, 18, 19), col=1:4)
```

Adicionalmente, se puede calcular la importancia de cada variable en las componentes. Esto ayuda para la elección de los representantes de cada clúster.
...
#Componente 4
melted_sort4<-melted_sort %>% filter(Var2=="Comp.9") %>% head(10)

ggplot(data = melted_sort4[melted_sort4$Var2 == "Comp.9",]) +
  theme(legend.position = "none", 
        axis.text.x= element_text(angle=45, hjust = 1), 
        axis.ticks.x = element_blank()) + 
  labs(x = "Variables ",
       y = "Relative importance in principle component",
       title = "Variables in PC9") +
  geom_bar(aes(x=reorder(Var1,-value_abs), y=value_abs, fill=Var1), stat="identity")

```

# Reconstrucción de representantes principales de los clusters
```{r}
...
center_recon = scale(center_recon, center = -mu, scale = FALSE)

cbind(center_recon, clusters_final$centers[(npc_opt+1):28])
```
